...
async def create_credit_note(
    ...
    try:
        ...
        for item_data in note.items:
            from app.models.vouchers.financial import CreditNoteItem  # Assuming CreditNoteItem is in financial.py
            item = CreditNoteItem(
        ...
        max_date_stmt = select(func.max(CreditNote.date)).where(
            CreditNote.organization_id == org_id,
            CreditNote.voucher_number.like(search_pattern),
            CreditNote.id != db_note.id,
        ...
        if max_date and db_note.date < max_date:
            logger.info(f"Detected backdated insert for CN {db_note.voucher_number} - triggering reindex")
            reindex_result = await VoucherNumberService.reindex_vouchers_after_backdated_insert(
                db, "CN", org_id, CreditNote, db_note.date, db_note.id
            )
...
async def update_credit_note(
    ...
    try:
        ...
        # Async-safe model_validate (with error handling)
        try:
            validated_note = CreditNoteInDB.model_validate(note)
        except Exception as validate_err:
            logger.error(f"Validation error post-load: {str(validate_err)}")
            # Fallback to dict serialization if Pydantic fails on rels
            validated_note = CreditNoteInDB.model_validate(note.__dict__)
