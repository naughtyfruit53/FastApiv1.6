...
async def check_backdated_conflict(
    ...
    try:
        ...
        stmt = select(func.max(PaymentVoucher.date)).where(
            PaymentVoucher.organization_id == org_id
        )
        result = await db.execute(stmt)
...
async def create_payment_voucher(
    ...
    try:
        ...
        conflict_info = await VoucherNumberService.check_backdated_voucher_conflict(
            db, "PMT", org_id, PaymentVoucher, db_voucher.date
        )
        ...
        db_voucher.entity = await load_entity(db, db_voucher.entity_id, db_voucher.entity_type)
