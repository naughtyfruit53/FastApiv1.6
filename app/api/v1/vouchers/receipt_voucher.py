...
from app.models.erp_models import ChartOfAccounts  # Assuming ChartOfAccounts is in erp_models.py
...
import logging
from sqlalchemy.sql import text  # Kept for text queries
    ...
    try:
        ...
        if not voucher_data.get('voucher_number') or voucher_data['voucher_number'] == '':
            voucher_data['voucher_number'] = await VoucherNumberService.generate_voucher_number_async(
                db, "RCT", org_id, ReceiptVoucher, voucher_date=voucher_date
            )
        else:
            stmt = select(ReceiptVoucher).where(
                ReceiptVoucher.organization_id == org_id,
        ...
        max_date_stmt = select(func.max(ReceiptVoucher.date)).where(
            ReceiptVoucher.organization_id == org_id,
            ReceiptVoucher.voucher_number.like(search_pattern),
            ReceiptVoucher.id != db_voucher.id,
            ReceiptVoucher.is_deleted == False
        )
        result = await db.execute(max_date_stmt)
        max_date = result.scalar()
        
        if max_date and db_voucher.date < max_date:
            logger.info(f"Detected backdated insert for RCT {db_voucher.voucher_number} - triggering reindex")
        ...
        try:
            validated_voucher = ReceiptVoucherInDB.model_validate(db_voucher)
        except Exception as validate_err:
            logger.error(f"Validation error post-load: {str(validate_err)}")
            # Fallback to dict serialization if Pydantic fails on rels
            validated_voucher = ReceiptVoucherInDB.model_validate(db_voucher.__dict__)
        
        await add_entity_name(validated_voucher, db)
        ...
        logger.info(f"Receipt voucher {validated_voucher.voucher_number} created by {current_user.email}")
...
async def update_receipt_voucher(
    ...
    try:
        ...
        logger.debug(f"Before commit for receipt voucher {voucher_id}")
        await db.commit()
        logger.debug(f"After commit for receipt voucher {voucher_id}")
        ...
        if not voucher:
            raise HTTPException(
                ...
                detail="Receipt voucher not found"
            )
        
        # Async-safe model_validate (with error handling)
        try:
            validated_voucher = ReceiptVoucherInDB.model_validate(voucher)
        except Exception as validate_err:
            logger.error(f"Validation error post-load: {str(validate_err)}")
            # Fallback to dict serialization if Pydantic fails on rels
            validated_voucher = ReceiptVoucherInDB.model_validate(voucher.__dict__)
        ...
        logger.info(f"Receipt voucher {validated_voucher.voucher_number} updated by {current_user.email}")
