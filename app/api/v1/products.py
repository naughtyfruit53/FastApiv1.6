...
# =============================================================================
# Item Rate Memory APIs
# =============================================================================

@router.get("/{product_id}/last-purchase-rate", response_model=dict)
async def get_last_purchase_rate(
    product_id: int,
    auth: tuple = Depends(require_permission_with_entitlement("erp", "products.read", "products")),
    db: AsyncSession = Depends(get_db)
):
    """
    Get the last purchase rate for a product.
    ...
    stmt = select(
        ...
        GoodsReceiptNoteItem.product_id == product_id,
...
@router.get("/{product_id}/last-sales-rate", response_model=dict)
async def get_last_sales_rate(
    product_id: int,
    ...
    """
    Get the last sales rate for a product.
    ...
    sv_stmt = select(
        ...
        SalesVoucherItem.product_id == product_id,
