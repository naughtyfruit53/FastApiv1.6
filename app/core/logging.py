...
def log_license_creation(org_name: str, org_admin_email: str, created_by: Optional[str] = None, success: bool = True, error: Optional[str] = None):
    """Log license creation events with comprehensive details"""
    logger = get_logger("license")
    status = "SUCCESS" if success else "FAILED"
    message = f"License Creation {status}: Organization '{org_name}' for admin {org_admin_email}"
    if created_by:
