...
from sqlalchemy import Column, Integer, String, Float, DateTime, Text, ForeignKey, Boolean, UniqueConstraint, Index, JSON, Date
...
# NEW: Added ProductionEntry model
class ProductionEntry(BaseVoucher):
    __tablename__ = "production_entries"

    production_order_id = Column(Integer, ForeignKey("manufacturing_orders.id"), nullable=False)  # FIXED: Added column
    ...
    downtime_events = Column(JSON)  # List of strings
    notes = Column(Text)
    ...
    production_order = relationship("ManufacturingOrder", back_populates="production_entries")  # FIXED: Changed back_populates to back_populates
    bom_consumption = relationship("ProductionEntryConsumption", back_populates="production_entry", cascade="all, delete-orphan")

# NEW: Added ProductionEntryConsumption model
class ProductionEntryConsumption(Base):
    __tablename__ = "production_entry_consumptions"
