...
export const dispatchService = {
  getCouriers: async () => {
    try {
      console.log('[DispatchService] Fetching couriers');
      const response = await api.get('/transport/couriers');
      console.log('[DispatchService] Successfully fetched couriers');
      return response;
    } catch (error: any) {
      console.error('[DispatchService] Error fetching couriers:', error);
      throw new Error(error.response?.data?.detail || 'Failed to fetch couriers');
    }
  },
  ...
  updatePurchaseOrderTracking: async (voucherId: number, trackingData: TrackingUpdate) => {
    try {
      ...
      console.error('[DispatchService] Error updating PO tracking:', error);
      throw new Error(error.response?.data?.detail || 'Failed to update PO tracking');
    }
  },
