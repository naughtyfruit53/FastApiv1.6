...
class ProfessionalPdfService {
  ...
  private async drawHeader(
    ...
    companyInfo.forEach((info) => {
      this.doc.text(info, this.pageWidth - this.margins.right, yPosition, {
        align: "right",
      });
      yPosition += 4;
    });
    ...
    this.doc.text(voucherTitle, this.pageWidth / 2, yPosition + 3, {
      align: "center",
    });
  ...
  private drawItemsTable(items: any[], yPosition: number): number {
    ...
    const tableData = items.map((item, index) => ({
      ...
      description:
        item.product?.name || item.description || `Product ${item.product_id}`,
  ...
  private drawFooter(yPosition: number): void {
    ...
    this.doc.setTextColor(0);
    this.doc.text(
      "For " + (this.doc as any).companyName || "Organization",
      this.pageWidth - this.margins.right - 60,
      footerY - 25,
    );
    ...
    this.doc.setLineWidth(0.5);
    this.doc.line(
      this.pageWidth - this.margins.right - 80,
      footerY - 5,
