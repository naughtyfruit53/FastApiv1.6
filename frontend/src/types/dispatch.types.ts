...
export type DispatchOrderStatus =
  (typeof DISPATCH_ORDER_STATUSES)[keyof typeof DISPATCH_ORDER_STATUSES];
export type DispatchItemStatus =
  (typeof DISPATCH_ITEM_STATUSES)[keyof typeof DISPATCH_ITEM_STATUSES];
export type InstallationJobStatus =
  (typeof INSTALLATION_JOB_STATUSES)[keyof typeof INSTALLATION_JOB_STATUSES];
export type InstallationJobPriority =
  (typeof INSTALLATION_JOB_PRIORITIES)[keyof typeof INSTALLATION_JOB_PRIORITIES];
...
export const hasInstallationManagementPermission = (
  userRole: string,
): boolean => {
  const allowedRoles = ["org_admin", "admin", "manager"];
  return allowedRoles.includes(userRole);
