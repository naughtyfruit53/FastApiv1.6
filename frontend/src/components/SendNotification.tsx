...
import { Send, Person, Group, Preview } from "@mui/icons-material";
...
const SendNotification: React.FC = () => {
  ...
  const sendBulkMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(
        `Bulk notification sent: ${data.successful_sends}/${data.total_recipients} successful`,
      );
    ...
    onError: (error: any) => {
      toast.error(
        error.response?.data?.detail || "Failed to send bulk notification",
      );
    },
  });
  // Preview mutation
  ...
  const handleSendBulk = () => {
    if (
      ...
      bulkForm.recipient_ids.length === 0
    ) {
    ...
    const request: BulkNotificationRequest = {
      ...
      recipient_ids:
        bulkForm.recipient_ids.length > 0 ? bulkForm.recipient_ids : undefined,
      segment_name: bulkForm.segment_name || undefined,
      variables:
        Object.keys(bulkForm.variables).length > 0
          ? bulkForm.variables
          : undefined,
    };
  ...
  const selectedTemplate = templates.find(
    (t) =>
      t.id ===
      parseInt(
        selectedTab === 0 ? singleForm.template_id : bulkForm.template_id,
      ),
  );
          ...
          <TabPanel value={selectedTab} index={0}>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={6}>
                <FormControl fullWidth margin="normal">
                  ...
                  <Select
                    ...
                    label="Channel"
                    onChange={(e) =>
                      setSingleForm({ ...singleForm, channel: e.target.value })
                    }
                  >
              ...
              <Grid item xs={12} md={6}>
                <Autocomplete
                  options={
                    singleForm.recipient_type === "customer" ? customers : users
                  }
                  getOptionLabel={(option) => option.name}
                  value={
                    entities.find(
                      (e) => e.id.toString() === singleForm.recipient_id,
                    ) || null
                  }
                  onChange={(_, value) =>
                    setSingleForm({
                  ...
                  <Grid item xs={12}>
                    <TextField
                      ...
                      onChange={(e) =>
                        setSingleForm({
                          ...singleForm,
                          override_subject: e.target.value,
                        })
                      }
                      margin="normal"
                    />
                  ...
                  <Grid item xs={12}>
                    <TextField
                      ...
                      onChange={(e) =>
                        setSingleForm({
                          ...singleForm,
                          override_content: e.target.value,
                        })
                      }
                      multiline
                      rows={4}
              ...
              <Grid item xs={12} md={6}>
                <FormControl fullWidth margin="normal">
                  ...
                  <Select
                    ...
                    label="Template (Optional)"
                    onChange={(e) =>
                      setBulkForm({ ...bulkForm, template_id: e.target.value })
                    }
                  >
                    <MenuItem value="">Custom Message</MenuItem>
                    {templates.map((template) => (
              ...
              <Grid item xs={12} md={6}>
                <FormControl fullWidth margin="normal">
                  ...
                  <Select
                    ...
                    label="Channel"
                    onChange={(e) =>
                      setBulkForm({ ...bulkForm, channel: e.target.value })
                    }
                  >
                    {NOTIFICATION_CHANNELS.map((channel) => (
                      <MenuItem key={channel} value={channel}>
                ...
                <Grid item xs={12}>
                  <Autocomplete
                    ...
                    value={customers.filter((c) =>
                      bulkForm.recipient_ids.includes(c.id),
                    )}
                    onChange={(_, value) =>
                      setBulkForm({
                        ...bulkForm,
              ...
              {bulkForm.recipient_type === "segment" && (
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={(e) =>
                      setBulkForm({ ...bulkForm, segment_name: e.target.value })
                    }
                    margin="normal"
                ...
                <Grid item xs={12}>
                  <Autocomplete
                    ...
                    value={users.filter((u) =>
                      bulkForm.recipient_ids.includes(u.id),
                    )}
                    onChange={(_, value) =>
                      setBulkForm({
                        ...bulkForm,
                ...
                <>
                  ...
                  <Grid item xs={12}>
                    <TextField
                      ...
                      onChange={(e) =>
                        setBulkForm({ ...bulkForm, content: e.target.value })
                      }
                      multiline
                      rows={4}
                      margin="normal"
      ...
      >
        ...
        <DialogContent>
          {previewContent && (
            <Box>
              <Typography variant="h6" gutterBottom>
                {previewContent.template_name} (
                {getChannelDisplayName(previewContent.channel)})
              </Typography>
              ...
              <Box>
                ...
                <Box display="flex" gap={1} flexWrap="wrap" mt={1}>
                  {Object.entries(previewContent.variables_used).map(
                    ([key, value]) => (
                      <Chip
                        key={key}
                        label={`${key}: ${value}`}
