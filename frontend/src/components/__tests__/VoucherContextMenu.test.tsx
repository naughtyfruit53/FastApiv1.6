...
const _mockVoucherWithCustomer = {
  id: 2,
...
describe("VoucherContextMenu", () => {
  ...
  it("opens menu when kebab button is clicked", async () => {
    ...
    const _kebabButton = screen.getByLabelText("voucher actions");
  ...
  it("calls onView when View is clicked", async () => {
    ...
    await waitFor(() => {
      const _viewButton = screen.getByText("View");
  ...
  it("calls onEdit when Edit is clicked", async () => {
    ...
    await waitFor(() => {
      const _editButton = screen.getByText("Edit");
  ...
  it("calls onEmail when email is clicked", async () => {
    ...
    await waitFor(() => {
      const _emailButton = screen.getByText("Send to Test Vendor");
  ...
  it("disables email when no recipient is available", async () => {
    ...
    await waitFor(() => {
      const _emailButton = screen.getByText("Send Email");
      expect(emailButton.closest("li")).toHaveAttribute(
        "aria-disabled",
        "true",
      );
    });
