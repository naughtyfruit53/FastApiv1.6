...
describe('UnifiedLoginForm', () => {
  ...
  it('renders default login form with email and password fields', () => {
    ...
    expect(screen.getByRole('checkbox', { name: /login with otp/i })).toBeInTheDocument();
  ...
  it('switches to OTP mode when checkbox is checked', () => {
    ...
    const otpCheckbox = screen.getByRole('checkbox', { name: /login with otp/i });
  ...
  it('handles standard email/password login', async () => {
    ...
    await waitFor(() => {
      expect(mockedAuthService.loginWithEmail).toHaveBeenCalledWith('test@example.com', 'password123');
  ...
  it('handles OTP verification and login', async () => {
    const mockOtpResponse = { message: 'OTP sent successfully', email: 'test@example.com' };
    ...
    await waitFor(() => {
      expect(mockedAuthService.verifyOTP).toHaveBeenCalledWith('test@example.com', '123456');
  ...
  it('handles errors gracefully', async () => {
    mockedAuthService.loginWithEmail.mockRejectedValue(new Error('Invalid credentials'));
