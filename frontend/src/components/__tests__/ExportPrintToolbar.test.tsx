...
describe("ExportPrintToolbar", () => {
  ...
  describe("Excel Export", () => {
    it("should call onExportExcel when Excel option is clicked", async () => {
      const mockExportExcel = jest.fn().mockResolvedValue(
        new Blob(["test"], {
    ...
    it("should handle export errors gracefully", async () => {
      const mockExportExcel = jest
        .fn()
        .mockRejectedValue(new Error("Export failed"));
      const consoleSpy = jest
        .spyOn(console, "error")
        .mockImplementation(() => {});
      render(<ExportPrintToolbar onExportExcel={mockExportExcel} />);
      const exportButton = screen.getByText("Export");
      fireEvent.click(exportButton);
      await waitFor(() => {
        const excelOption = screen.getByText("Export to Excel");
  ...
  describe("Conditional Rendering", () => {
    ...
    it("should only show Excel option when showCSV is false", async () => {
      const mockExportExcel = jest.fn();
      render(
        <ExportPrintToolbar onExportExcel={mockExportExcel} showCSV={false} />,
      );
      const exportButton = screen.getByText("Export");
      fireEvent.click(exportButton);
      await waitFor(() => {
    ...
    it("should only show CSV option when showExcel is false", async () => {
      const mockExportCSV = jest.fn();
      render(
        <ExportPrintToolbar onExportCSV={mockExportCSV} showExcel={false} />,
      );
      const exportButton = screen.getByText("Export");
      fireEvent.click(exportButton);
      await waitFor(() => {
  ...
  describe("Loading and Disabled States", () => {
    it("should disable buttons when disabled prop is true", () => {
      const mockExportExcel = jest.fn();
      render(
        <ExportPrintToolbar onExportExcel={mockExportExcel} disabled={true} />,
      );
    ...
    it("should show loading indicator when loading is true", () => {
      const mockExportExcel = jest.fn();
      render(
        <ExportPrintToolbar onExportExcel={mockExportExcel} loading={true} />,
      );
      expect(screen.getByText("Export")).toBeDisabled();
      expect(screen.getByLabelText("Print report")).toBeDisabled();
    ...
    it("should disable export options during export", async () => {
      const mockExportExcel = jest
        ...
        .mockImplementation(
          () =>
            new Promise((resolve) =>
              setTimeout(() => resolve(new Blob(["test"])), 100),
            ),
        );
      render(<ExportPrintToolbar onExportExcel={mockExportExcel} />);
      const exportButton = screen.getByText("Export");
