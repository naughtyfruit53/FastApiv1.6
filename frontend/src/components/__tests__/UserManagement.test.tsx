...
const mockOrganizationService = organizationService as jest.Mocked<
...
describe("UserManagement Component", () => {
  ...
  test("should render user management page with organization context", async () => {
    ...
    expect(
      screen.getByText(/Managing users for your organization/),
    ).toBeInTheDocument();
  ...
  test("should call organization-scoped API to fetch users", async () => {
    ...
    await waitFor(() => {
      expect(mockOrganizationService.getOrganizationUsers).toHaveBeenCalledWith(
        1,
      );
    });
  });
  ...
  test("should show access denied for standard users", () => {
    ...
    jest
      .mocked(require("../context/AuthContext").useAuth)
      .mockReturnValue(standardUserContext);

    render(<UserManagement />, { wrapper: createWrapper() });

    expect(
      screen.getByText(/You don't have permission to manage users/),
    ).toBeInTheDocument();
  });
});
...
describe("OrganizationService Integration", () => {
  test("should call correct organization-scoped endpoints", () => {
    ...
    expect(
      mockOrganizationService.createUserInOrganization,
    ).toHaveBeenCalledWith(orgId, userData);

    mockOrganizationService.updateUserInOrganization(orgId, userId, userData);
    expect(
      mockOrganizationService.updateUserInOrganization,
    ).toHaveBeenCalledWith(orgId, userId, userData);

    mockOrganizationService.deleteUserFromOrganization(orgId, userId);
    expect(
      mockOrganizationService.deleteUserFromOrganization,
    ).toHaveBeenCalledWith(orgId, userId);

    mockOrganizationService.getOrganizationInvitations(orgId);
    expect(
      mockOrganizationService.getOrganizationInvitations,
    ).toHaveBeenCalledWith(orgId);

    mockOrganizationService.resendInvitation(orgId, 789);
    expect(mockOrganizationService.resendInvitation).toHaveBeenCalledWith(
      orgId,
      789,
    );

    mockOrganizationService.cancelInvitation(orgId, 789);
    expect(mockOrganizationService.cancelInvitation).toHaveBeenCalledWith(
      orgId,
      789,
    );
  });
