...
describe("CompanyLogoUpload", () => {
  ...
  it("renders upload area when no logo exists", () => {
    renderWithProviders(
      <CompanyLogoUpload companyId={1} currentLogoPath={null} />,
    );

    expect(screen.getByText("Company Logo")).toBeInTheDocument();
    expect(
      screen.getByText("Drag & drop logo here or click to upload"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("PNG, JPG, JPEG, GIF up to 5MB"),
    ).toBeInTheDocument();
  });

  it("renders logo preview when logo exists", () => {
    renderWithProviders(
      <CompanyLogoUpload companyId={1} currentLogoPath="/path/to/logo.png" />,
    );
  ...
  it("shows error for invalid file type", async () => {
    ...
    const input = screen.getByRole("button", {
      name: /drag & drop logo here or click to upload/i,
    });
  ...
  it("shows error for oversized file", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/logo file size must be less than 5mb/i),
      ).toBeInTheDocument();
    });
  });
