...
const mockGetAccountBalance = getAccountBalance as jest.MockedFunction<
...
describe("BalanceDisplay", () => {
  ...
  it("displays customer balance - positive amount (receivable)", async () => {
    ...
    render(<BalanceDisplay accountType="customer" accountId={1} />, {
      wrapper: createWrapper(),
    });

    await waitFor(() => {
      expect(
        screen.getByText("Current Balance: ₹5,000 receivable"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("displays customer balance - negative amount (advance)", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Current Balance: ₹2,000 advance"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("displays vendor balance - negative amount (payable)", async () => {
    ...
    render(<BalanceDisplay accountType="vendor" accountId={1} />, {
      wrapper: createWrapper(),
    });

    await waitFor(() => {
      expect(
        screen.getByText("Current Balance: ₹3,000 payable"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("displays vendor balance - positive amount (advance)", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Current Balance: ₹1,500 advance"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("handles zero balance", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Current Balance: ₹0 advance"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("formats large amounts with comma separation", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Current Balance: ₹123,456.78 receivable"),
      ).toBeInTheDocument();
    });
  });
});
