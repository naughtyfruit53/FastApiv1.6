...
const SLAStatus: React.FC<SLAStatusProps> = ({
  slaTracking,
  variant = "compact",
}) => {
    ...
    return (
      ...
      >
        <Tooltip title={`Response SLA: ${slaTracking.response_status}`}>
          <Chip
            icon={getStatusIcon(
              slaTracking.response_status,
              responseTimeInfo.isOverdue,
            )}
        ...
        <Tooltip title={`Resolution SLA: ${slaTracking.resolution_status}`}>
          <Chip
            icon={getStatusIcon(
              slaTracking.resolution_status,
              resolutionTimeInfo.isOverdue,
            )}
          ...
          <Grid item xs={12} md={6}>
            <Stack spacing={1}>
              ...
              <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <Typography variant="caption">
                  {responseTimeInfo.isCompleted
                    ? "Completed"
                    : responseTimeInfo.isOverdue
                      ? `Overdue by ${responseTimeInfo.hours.toFixed(1)}h`
                      : `${responseTimeInfo.hours.toFixed(1)}h remaining`}
                </Typography>
              </Box>

              {slaTracking.response_breach_hours !== null &&
                slaTracking.response_breach_hours !== undefined && (
                  <Typography
                    variant="caption"
                    color={
          ...
          <Grid item xs={12} md={6}>
            <Stack spacing={1}>
              ...
              <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <Typography variant="caption">
                  {resolutionTimeInfo.isCompleted
                    ? "Completed"
                    : resolutionTimeInfo.isOverdue
                      ? `Overdue by ${resolutionTimeInfo.hours.toFixed(1)}h`
                      : `${resolutionTimeInfo.hours.toFixed(1)}h remaining`}
                </Typography>
              </Box>

              {slaTracking.resolution_breach_hours !== null &&
                slaTracking.resolution_breach_hours !== undefined && (
                  <Typography
                    variant="caption"
                    color={
          ...
          <Grid item xs={12}>
            ...
            >
              <Typography variant="caption" color="textSecondary">
                Policy: {slaTracking.policy.name} | Escalation:{" "}
                {slaTracking.policy.escalation_enabled ? "Enabled" : "Disabled"}{" "}
