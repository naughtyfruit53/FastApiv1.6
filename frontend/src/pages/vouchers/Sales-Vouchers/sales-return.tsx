...
const SalesReturnPage: React.FC = () => {
  ...
  const handleEditWithData = (voucher: any) => {
    ...
    queryClient.setQueryData(['sales-return', voucher.id], voucher);
  };
      ...
      if (currentDate && mode === 'create') {
        try {
          ...
          const response = await axios.get(
            `/api/v1/sales-returns/next-number?voucher_date=${currentDate}`
          );
          setValue('voucher_number', response.data);
          
          // Check for backdated conflicts
          const conflictResponse = await axios.get(
            `/api/v1/sales-returns/check-backdated-conflict?voucher_date=${currentDate}`
          );
      ...
      <Table stickyHeader size="small">
        ...
        <TableBody>
          {latestVouchers.length === 0 ? (
            <TableRow><TableCell colSpan={5} align="center">No sales returns available</TableCell></TableRow>
          ) : (
              ...
              <TableRow key={voucher.id} hover onContextMenu={(e) => { e.preventDefault(); handleContextMenu(e, voucher); }} sx={{ cursor: "pointer" }}>
                ...
                <TableCell align="right" sx={{ fontSize: 12, p: 0 }}>
                  <VoucherContextMenu
                    ...
                    onDuplicate={(id) => handleDuplicate(id, voucherList, reset, setMode, "Sales Return")}
                    showKebab={true}
                    onClose={() => {}}
                  />
                </TableCell>
