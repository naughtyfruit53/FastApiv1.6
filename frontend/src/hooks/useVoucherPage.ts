...
export const useVoucherPage = (config: VoucherPageConfig) => {  
  ...
  const schema = useMemo(() => {
    ...
    if (config.entityType === 'purchase') {
      Object.assign(baseSchema, { vendor_id: yup.number().required('Vendor is required') });
    } else if (config.entityType === 'sales') {
      Object.assign(baseSchema, { customer_id: yup.number().required('Customer is required') });
    }

    if (config.hasItems !== false) {
      let itemSchema;
      if (config.voucherType === 'goods-receipt-notes') {
        // GRN-specific item schema (no 'quantity', use received/accepted/rejected)
        itemSchema = yup.object({
          product_id: yup.number().required('Product is required'),
          ordered_quantity: yup.number().min(0).required('Ordered quantity is required'),
          received_quantity: yup.number().min(0).required('Received quantity is required'),
          accepted_quantity: yup.number().min(0).required('Accepted quantity is required'),
          rejected_quantity: yup.number().min(0).required('Rejected quantity is required'),
          unit_price: yup.number().min(0).required('Unit price is required'),
        });
      } else {
        // Standard item schema for other vouchers
        itemSchema = yup.object({
          product_id: yup.number().required('Product is required'),
      ...
      Object.assign(baseSchema, {
        items: yup.array().min(1, 'At least one item is required').of(itemSchema),
      });
    }
  ...
  const defaultValues = useMemo(() => {
    ...
    if (config.hasItems === false) {
      ...
      if (config.voucherType === 'goods-receipt-notes') {
        // GRN-specific default item (override with GRN fields)
        items = [{
          ...
          unit_price: 0.0,
          unit: "",
          po_item_id: null as number | null,
        }];
      }
      return {
        ...baseValues,
  ...
  }, [config.entityType, config.hasItems, config.voucherType]);
  ...
  const reviseMutation = useMutation({
    mutationFn: (data: any) =>
      voucherService.createRevision(
        config.apiEndpoint || config.voucherType,
        selectedId!,
        data,
      ),
    onSuccess: async (newVoucher) => {
      console.log("[useVoucherPage] Voucher revised successfully:", newVoucher);
      queryClient.setQueryData(
        [config.voucherType, currentPage, pageSize],
        (oldData: any) => {
          if (!oldData) {
    ...
    onError: (error: any) => {
      console.error("[useVoucherPage] Error revising voucher:", error);
      handleVoucherError(error, "revise");
    },
  });
  ...
  useEffect(() => {
    if (mode === "revise" && nextRevisionNumber) {
