...
describe("Ledger Report", () => {
  ...
  describe("Ledger Tab Access", () => {
    ...
    it("shows access denied message when user lacks permission", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(
            /You don't have permission to access the Ledger report/,
          ),
        ).toBeInTheDocument();
      });
    });
  ...
  describe("Outstanding Ledger View", () => {
    ...
    it("displays outstanding ledger data with proper visual distinction", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText("Total Payable: ₹3,000 (Amount owed to vendors)"),
        ).toBeInTheDocument();
        expect(
          screen.getByText(
            "Total Receivable: ₹5,000 (Amount owed by customers)",
          ),
        ).toBeInTheDocument();
  ...
  describe("Error Handling", () => {
    ...
    it("displays error message when complete ledger fails to load", async () => {
      mockReportsService.getCompleteLedger.mockRejectedValue(
        new Error("API Error"),
      );
      ...
      await waitFor(() => {
        expect(
          screen.getByText("No complete ledger data available"),
        ).toBeInTheDocument();
      });
    });

    it("displays error message when outstanding ledger fails to load", async () => {
      mockReportsService.getOutstandingLedger.mockRejectedValue(
        new Error("API Error"),
      );
      ...
      await waitFor(() => {
        expect(
          screen.getByText("No outstanding ledger data available"),
        ).toBeInTheDocument();
      });
    });
  });
